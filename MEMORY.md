...
- Final validation passed with 30 Vitest files / 51 tests, `pnpm check`, and the production build. The build retains only the existing non-blocking large-chunk warning. Live restore point `139fe0c7` was saved, and its matching source commit `139fe0c` was pushed to `github/main`.

## Expanded Syllables, Seasonal Care, and Mobile Navigation Verification
- Direct browser interaction confirmed the Turtle rescue now moves calmly through three separate, picture-led patterns: turtle (two claps), acorn (two claps), and butterfly (three claps). Each correct selection reveals only the next pattern after a short acknowledgement, preserving a single clear action at a time.
- Direct summer Weather Wonder inspection confirmed that Splash’s plush avatar and “Cozy care for Splash” context appear before the two ordered care actions. Completing the leafy-shade action turns it into a readable acknowledgement and unlocks Splash’s water-bowl action, preserving the existing one-action-at-a-time interaction model.
- Expanded release validation passed with 31 Vitest files / 54 tests, `pnpm check`, and a production build. The build retains the existing non-blocking large-chunk warning only. Parent-value recommendations are recorded in `PARENT_VALUE_RECOMMENDATIONS.md`, grounded in current healthy early-childhood media-use guidance and deliberately preserving local-only privacy, optional adult participation, and no-pressure play.
- Calm ritual / celebration-path verification: generated and uploaded the 118-second `critter-rescue-calm-ritual_084abd92.wav` instrumental loop. It is parent-enabled only and will not start from navigation or a stored preference alone. Added four parent-selectable seasonal paths, keeping formal holiday names in Parent Settings and the child flow to neutral visual trail names. Direct reduced-motion Lights & kindness testing verified one large action at a time, then a gentle completion card. The other three phone-size visual paths were checked for distinct, calm pumpkin-lantern, harvest, and winter-sparkle presentation.
- Final calm-ritual validation: Parent Settings visibly provides a disabled-by-default sound toggle, an explicit Calm shared ritual selection, a low 0–45% level control, and an explicit Start selected sound action. After testing, the local preview was reset to default preferences. `pnpm check`, 34 Vitest files / 59 tests, and the production build passed; the build retains only the established non-blocking large-chunk warning.
- Sanctuary UI clarity pass: the first-play overlay is now a semantic dialog with deeper dimming and a calm 3px backdrop blur. It presents only the requested title, “We’ll do this together.” reassurance, optional visible Listen action, three consistent line-icon steps, one coral “Start the Rescue.” primary control, and an outlined “Explore First” secondary control. Header icon buttons now use Lucide line icons, titles, and labels; broken header logo and plush assets render compact text fallbacks rather than clipped alt text. Direct laptop (1440×960), tablet (768×1024), normal phone (375×812), and Reduce Motion phone views were checked. `pnpm test` passed 36 files / 63 tests; `pnpm check` and build passed with the standard non-blocking chunk-size warning.
- Child-friendly trail, parent tip, and loading pass: the trail picker is a semantic dialog that highlights exactly one “Ready now” card, uses plain non-numeric language for locked paths, and preserves the existing rescue callback after its gentle transition. Parent Settings now begins with a grown-up-only co-play tip that suggests one rescue, one child-led trail choice, and one kind reflection without a performance expectation. A generated wordless plush-fox-and-lantern storybook scene now supports the loading state; loading artwork and its dots respect the saved Reduce Motion preference. Direct phone, tablet, and laptop checks covered the zones, tip, and loading route, including a Reduce Motion phone view. `pnpm test` passed 38 files / 65 tests; `pnpm check` and build passed with the existing non-blocking chunk-size warning.
- Nutty optional-direction playback repair: the onboarding recording URL returned HTTP 200 directly, but the original helper silently swallowed browser playback failures and supplied no acknowledgement. Direction taps now explicitly preload the real clip, reset playback position, await `play()`, and report success/failure to the component. The button remains fully optional and does not autoplay; a concise “Nutty is speaking.” status gives a child visible confirmation after a successful tap, while the existing caption remains available if playback is unavailable. Tests cover the explicit component tap and the actual onboarding clip instantiation/start path. `pnpm test` passed 40 files / 67 tests; TypeScript and the production build passed, with only the established non-blocking chunk-size warning.
- Gentle replay and comfort feedback: after the first optional direction tap, a compact “Ask a grown-up” comfort-volume panel lets the family choose “Play sound” or “Read together.” Choosing Play sound records the one-time confirmation locally; no audio begins beforehand, and captions/visual gameplay remain available. The actual audio-ended event converts the Listen button to an explicit Replay directions control; it never replays automatically. While a clip is active, a small Nutty squirrel badge pulses only when motion is permitted and remains still with Reduce Motion. Regression coverage now includes first-use confirmation, replay state, active speaking badge, and the still Reduce Motion variant. `pnpm test` passed 40 files / 69 tests; `pnpm check` and production build passed with the standard non-blocking chunk-size warning.
- Holiday Edition and beta-feedback preview: Parent Settings renders the adult-facing Holiday Edition switch, four individual celebration-path choices, and a parent-only feedback entry. Direct dialog inspection showed the privacy reminder, two large feedback-category controls, a large text field, and a single Send feedback action; no fabricated feedback was submitted. The original screen-capture utility continued to return blank query-preview frames after a dependency optimization, but direct browser inspection verified that the page renders. The Parent Settings header now uses the Safari safe-area inset, while camp header icon controls and all game-screen buttons use a 44px minimum touch target.
- Holiday Edition activation: turning on the grown-up Holiday Edition switch selected Pumpkin & lantern as the initial calm trail and enabled the four-choice celebration selector. The parent-facing labels remain visible only in Parent Settings; the child-facing camp cue is intentionally neutral, “Cozy celebration.”
- Holiday Edition, beta feedback, and Safari touch refinement: the Holiday Edition remains a parent-selected collection built from the four researched individual paths; it retains the existing neutral child-facing trail cue and does not add pressure, scoring, or child-directed commerce. `beta_feedback` is a minimal persisted table with only feedback type, parent-entered note, non-identifying source context, and timestamp. The modal asks adults not to include a child’s name, photo, voice, or private details, validates type and message server-side, and uses a single deliberate submission. The Vite preview fallback remains subject to the host runtime’s empty 400 response behavior for raw POSTs, but the production Express endpoint returns correctly typed validation JSON and the shared validator is covered in the regular suite. Parent Settings now has a Safari safe-area header, and game controls use 44px minimum targets plus `touch-action: manipulation`; manual iPhone-sized first-play review and direct iPad-layout inspection passed. Final validation: 42 test files / 72 tests, TypeScript, and production build pass with only the established non-blocking chunk-size warning.
- River Rescue foundation: Clover’s new Riverside mission uses a reusable three-step rescue contract: Bridge log → Rescue rope → Safe trail sign. The activity presents one active tool at a time, leaves each tool visible but shows gentle “useful soon” retry language, and uses the normal no-pressure completion flow. A fresh approved Nutty direction clip is stored at `/manus-storage/direction-riverRescue_e6a71518.mp3`. The direct 375×812 normal and Reduce Motion previews show a readable intro with one primary entry action; deterministic tests cover data order, mission wiring, the optional direction map, and the full visual tool sequence. A documented ten-rescue roadmap and parent-reviewed AI-assisted authoring process explicitly prohibit unreviewed in-child generation. Final validation: 44 test files / 75 tests, TypeScript, and production build passed; tests emit only non-failing React test-environment `act(...)` warnings in the new timing test, while the production build retains the standard non-blocking chunk-size warning.
- River Rescue expanded viewport check: the tablet (768×1024) and laptop (1280×720) first-play views both keep Clover’s card centered, the three helper pictures recognizable behind the overlay, and the one coral “I’ll help!” action clearly separated from optional listening. No controls or story text are clipped.

## Nest-Building Route Rescue Verification
- Wren’s new Deep Woods final mission uses the same reusable sequenced-rescue contract as River Rescue and teaches a calm spatial-planning route: Sturdy branch path → Soft moss bundle → Cozy nest cup. The active helper is marked clearly, completed helpers remain visible as checkmarks, and an out-of-order choice gives gentle “useful soon” guidance rather than a penalty.
- A fresh storybook nest-route reference is recorded in `ASSETS.md` at `/manus-storage/critter-rescue-nest-route-reference_b1e89ab7.png`. The matching optional Nutty prompt, “Tap the branch, then moss, then nest,” is stored at `/manus-storage/direction-nestRescue_a4a6c566.mp3`; the preview storage route resolves to HTTP 200 after its normal redirect. It is only invoked by the existing Listen control after a player or family tap.
- The deterministic interaction test covers intro entry, a gentle wrong moss attempt, branch → moss → nest sequencing, and the standard Back to Camp completion flow. Shared optional-direction coverage confirms that disabling spoken directions removes the Listen control without removing the readable Nest Rescue instruction; the shared player control also retains first-use volume choice, non-autoplay, and replay behavior.
- Responsive screenshot checks of the live preview covered normal laptop (1280×720), tablet (768×1024), phone (375×812), and laptop Reduce Motion views. Wren’s centered introduction, optional Listen control, and one coral “I’ll help!” entry action remain readable and unclipped. Global `.reduce-motion-mode` and system reduced-motion rules reduce every nested transition duration to 0.01ms while preserving interactions and acknowledgement states.
- Final automated validation passed with 46 Vitest files / 79 tests, `pnpm check`, and a production build. The build retains only the established non-blocking large-chunk warning.

## Lodge Rescue and Activity-Library Expansion Verification
- Bark’s Deep Woods Lodge-Building Rescue adds a third reusable picture-led planning module: Sturdy stick bundle → Soft leaf lining → Round lodge door. It keeps all three helpers visible, marks only the current helper, uses calm out-of-order guidance, and leads to the ordinary no-pressure completion flow. Its optional player-initiated Nutty clip is stored at `/manus-storage/direction-lodgeRescue_97316825.mp3`.
- Three additional curated activities are now in the ordinary mission library. Brook’s Riverside Telling Time activity introduces only full-hour reading with a seven o’clock target and three large clock pictures. Daisy’s Sunny Meadow Garden Sorting activity shows one berry-or-leaf picture at a time with two large matching baskets. Summit’s Mountain Cozy Block Builder uses original unbranded rounded blocks in a steady base → walls → roof order; it is not affiliated with or labelled as any third-party brick product.
- Storybook authoring references are recorded in `ASSETS.md` for Bark’s lodge, the full-hour clock, the garden baskets, and the original block cottage. The new optional direction clips (`tellingTime`, `gardenSort`, and `brickBuild`) each resolve through the running preview at HTTP 200 and inherit the existing explicit-tap, volume-choice, caption, and replay flow.
- Deterministic data and RescueScreen tests cover a gentle incorrect attempt, each intended sequence or match, and standard Back to Camp completion. Responsive screenshot checks covered all three new introductions at laptop (1280×720), phone (375×812), and tablet (768×1024) with Reduce Motion. The compact layouts remain centered, readable, and unclipped; all new nonessential transitions inherit the global reduced-motion safeguard.
- Final automated validation passed with 54 Vitest files / 91 tests, `pnpm check`, and a production build. The build retains only the established non-blocking large-chunk warning.

## Ninth-Task Visibility Repair
- Source inspection confirmed that the new missions were already correctly reachable in ordinary play: `handleStartRescue` launches the live `zoneTaskProgress` index, and each zone’s task count matches its mission array. The user-facing trail drawer showed only unlabeled dots, which made the added final task difficult to count in the deployed game.
- The zone selector now visibly states each unlocked path’s current count, such as “3 of 12 rescues” and “0 of 11 rescues,” while retaining the dot trail and calm Ready now cue. Its accessible label also announces the count. Focused coverage, the full 54-file/91-test suite, TypeScript, and production build pass; a mobile preview confirms the count is readable without crowding the touch target. The only build message remains the established non-blocking chunk-size warning.

## Parent Activity Guide and Creative Block Studio
- Initial browser review confirms that the parent-only Activity Library explains the featured rescue activities, where each appears in a trail, and one gentle co-play prompt, then offers an explicit entry to the separate Cozy Block Studio. The child studio presents nine large open spaces, six original rounded block choices, a clear reset action, and visible no-score/no-wrong-build wording.
- Direct browser interaction confirms that choosing an original Coral wall block updates the child-facing instruction, then tapping an open space places that block on the mat and gives an affirming next-step prompt. The studio permits repeated block choices and has no failure, scoring, or time-pressure path.
- The parent guide is available only from Parent Settings, while the separate child-facing studio returns to that parent guide without modifying rescue progress. Component coverage verifies the parent entry and a choose → place → clear creative-build path. Full validation passed with 56 Vitest files / 93 tests, `pnpm check`, and production build; the established non-blocking chunk-size warning remains. Browser and screenshot checks confirm readable phone, tablet, and laptop layouts, plus Reduce Motion parity for the studio.
