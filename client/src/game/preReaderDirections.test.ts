...
describe('pre-reader spoken direction inventory', () => {
  it('covers onboarding, the daily trail, all rescue action types, and three learning rounds', () => {
    const required = ['onboarding', 'dailyTrail', 'bridge', 'clearPath', 'shelter', 'guidePath', 'memory', 'pattern', 'maze', 'gather', 'tracing', 'sorting', 'counting', 'shapeFit', 'spotDifference', 'sequence', 'findTools', 'colorMatch', 'sizeOrdering', 'critterPath', 'alliteration', 'habitatMatch', 'syllableClap', 'riverRescue', 'nestRescue', 'weatherWonder', 'teamRescue', 'learningColor', 'learningShape', 'learningPattern'] as const;
