...
function PrivacyContent() {
  return <>
    ...
    <Section title="Adult beta feedback"><p>If a grown-up chooses to send beta feedback, the selected feedback type, message, and short technical context are sent to the feedback service so the team can review a bug report or suggestion. Adults are asked not to include a child’s name, image, voice, contact details, or other private information. This feature is disabled in the native beta build until a parental gate is complete.</p><p>Before a public store release, the operator will publish a final retention schedule, a monitored support contact, and a deletion-request process for any adult-submitted feedback.</p></Section>
...
function FAQContent() {
  const faqs = [
    ...
    ['How does beta feedback work?', 'It is for grown-ups only. Please use it only for a short bug report or suggestion, and do not include private information about a child. Native beta feedback stays unavailable until a parental gate is implemented.'],
    ['How can a parent get help?', 'During the current private beta, use the grown-up beta feedback area for non-sensitive questions. Before public release, this page will list a monitored support email and formal privacy-request route.'],
  ] as const;
...
export default function ParentLegalScreen({ page, onBack, onNavigate }: Props) {
  ...
  return <div className="game-screen forest-bg overflow-y-auto px-4 pb-8">
    <header className="mx-auto flex w-full max-w-2xl items-center justify-between gap-3 pb-4 pt-[max(1rem,env(safe-area-inset-top))]"><div><p className="font-body text-[10px] font-bold uppercase tracking-[.14em] text-white/70">Grown-up information</p><h1 className="font-display text-2xl font-bold text-white">{heading}</h1></div><button onClick={() => { playButton(); onBack(); }} className="paper-card shrink-0 px-3 py-2 font-body text-sm text-[#2D2418] active:scale-95">Back to Parent Settings</button></header>
