      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6">
        ...
        {rescued.length === 0 ? (
          ...
          <section className="mt-4"><h3 className="font-display font-bold text-white text-lg mb-2 drop-shadow">Rescued Friends</h3><div className="grid grid-cols-2 gap-3">
            {rescued.map((critter, index) => <div key={`${critter.name}-${index}`} className="paper-card p-3 flex flex-col items-center gap-2 animate-rise-in" style={{ animationDelay: `${index * 40}ms` }}><CritterAvatar type={critter.type} size={64} expression="happy" animate /><p className="font-display font-bold text-[#2D2418] text-base">{critter.name}</p><p className="text-[#5C4D3C] text-xs font-body italic text-center">{critter.personality}</p><p className="text-[#5C4D3C] text-xs font-body text-center leading-snug">{critter.thanksLine}</p></div>)}
          </div></section>
        )}
