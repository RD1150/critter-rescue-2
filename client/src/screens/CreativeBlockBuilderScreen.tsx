...
import { loadParentBuildPrompts, type ParentBuildPrompt } from '../game/parentBuildPrompts';
...
export default function CreativeBlockBuilderScreen({ onBack }: { onBack: () => void }) {
  ...
  const [parentPrompts] = useState<ParentBuildPrompt[]>(() => loadParentBuildPrompts().filter((prompt) => prompt.enabled));
  const [selectedParentPromptId, setSelectedParentPromptId] = useState<string | null>(null);
  ...
  const chooseOwnIdea = () => { playButton(); setSelectedPromptId(null); setSelectedParentPromptId(null); setMessage('Your own idea is ready. Pick any block and make it your way.'); };
  const chooseParentPrompt = (prompt: ParentBuildPrompt) => { playButton(); setSelectedPromptId(null); setSelectedParentPromptId(prompt.id); setMessage(`${prompt.prompt} You can use any blocks you like.`); };
  const saveCreation = () => {
    playButton();
  ...
  return <div className="game-screen forest-bg overflow-y-auto px-4 pb-8">
    ...
    <main className="mx-auto flex w-full max-w-lg flex-col items-center gap-3">
      ...
      <section className="w-full rounded-2xl border border-[#DBC7E7] bg-[#F8F0FB] px-3 py-3 shadow-sm" aria-labelledby="parent-picked-title" hidden={parentPrompts.length === 0}><p className="font-body text-[10px] font-bold uppercase tracking-[.14em] text-[#76538B]">Grown-up picked ideas</p><h2 id="parent-picked-title" className="font-display text-base font-bold text-[#2D2418]">A special build idea</h2><p className="mt-1 font-body text-[11px] leading-snug text-[#5F5268]">A grown-up chose these optional ideas for you. You can still build your own way.</p><div className="mt-3 flex snap-x gap-2 overflow-x-auto pb-1" aria-label="Grown-up picked build ideas">{parentPrompts.map((prompt) => <button key={prompt.id} type="button" onClick={() => chooseParentPrompt(prompt)} aria-pressed={selectedParentPromptId === prompt.id} aria-label={`${prompt.title}. ${prompt.prompt}`} className={`w-40 shrink-0 snap-start rounded-2xl border-2 p-3 text-left shadow-sm active:scale-95 ${selectedParentPromptId === prompt.id ? 'border-[#A98BC5] bg-white ring-2 ring-[#E9D7F1]' : 'border-[#DBC7E7] bg-white/80'}`}><span aria-hidden="true" className="mb-2 flex h-9 w-9 items-center justify-center rounded-xl bg-[#A98BC5] text-xl text-white">✦</span><strong className="block font-display text-sm text-[#2D2418]">{prompt.title}</strong><span className="mt-1 block font-body text-[11px] leading-snug text-[#5F5268]">{prompt.prompt}</span></button>)}</div></section>
