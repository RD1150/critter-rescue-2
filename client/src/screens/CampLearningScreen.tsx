...
export default function CampLearningScreen({ onBack }: Props) {
  ...
  return <div className="game-screen forest-bg overflow-y-auto px-4 pb-7"><header className="flex items-center justify-between pt-safe pt-4 pb-3"><div className="flex items-center gap-2"><CritterAvatar type="squirrel" size={48} expression="happy" /><div><p className="font-body text-[10px] uppercase tracking-[.14em] text-white/65">Camp Learning Trail</p><h1 className="font-display text-xl font-bold text-white">Colors, shapes & patterns</h1></div></div><button onClick={onBack} className="paper-card px-3 py-1.5 text-sm font-body text-[#2D2418] active:scale-95">Camp</button></header>
    <main className="mx-auto max-w-md"><div className="flex gap-1.5 justify-center mb-3">{LEARNING_ROUNDS.map((_, index) => <span key={index} className={`w-8 h-2 rounded-full ${index < round ? 'bg-[#F3C94A]' : index === round ? 'bg-[#E66B5B]' : 'bg-white/25'}`} />)}</div><section className="paper-card p-5 text-center" style={{ borderTop: '3px solid #E66B5B' }}><p className="font-body text-[10px] uppercase tracking-[.15em] text-[#E66B5B] font-bold">{current.title} · Game {round + 1} of 3</p><h2 className="font-display text-2xl text-[#2D2418] font-bold mt-2">{current.prompt}</h2>{current.sequence && <div className="mt-5 flex items-end justify-center gap-3"><>{current.sequence.map((tile, index) => <ShapeTile key={index} tile={tile} small />)}</><span className="font-display text-4xl text-[#5C4D3C]">?</span></div>}<p className="font-body text-xs text-[#5C4D3C] mt-4">{message}</p><div className="grid grid-cols-2 gap-3 mt-5">{choices.map((tile, index) => <button key={`${tile.color}-${tile.shape}-${index}`} onClick={() => choose(tile)} className="min-h-[108px] rounded-2xl flex items-center justify-center bg-[#FAF3E5] border-2 border-[#E2D2BA] active:scale-95 transition-transform" aria-label={`Choose ${tile.colorName} ${tile.shape}`}><ShapeTile tile={tile} /></button>)}</div></section><p className="font-body text-center text-xs text-white/65 mt-4">Tap the picture that matches the question. Grown-ups can play too!</p></main></div>;
}
