...
export default function PreReaderDirection({ directionKey, compact = false, className = '' }: Props) {
  ...
  if (!preferences.spokenDirectionsEnabled) {
    ...
    return <div className={`rounded-2xl px-3 py-2 text-left ${className}`} style={{ background: 'rgba(248,232,216,.96)', border: '1px solid #E2C9AB' }}><p className="font-body text-[10px] uppercase tracking-[.12em] text-[#A85C41] font-bold">Try this</p><p className="font-display text-[#2D2418] text-sm leading-snug">“{line}”</p></div>;
  }
